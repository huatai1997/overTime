...
static void intern_json_free_value ( json_t **value )
{
	assert ( value != NULL );
	assert ( ( *value ) != NULL );
...
state1:			/* open value */
	{
		...
		switch ( cursor->type )
		{
			case JSON_STRING:
				...
				for(i=0; i<tmp_len; i++)
				{
					if(cursor->text[i] == '\"' || cursor->text[i] == '\\')
					{
					...
					if ( rcs_catc ( output,  cursor->text[i]) != RS_OK )
					{
						return JSON_MEMORY;
					}
				}
...
char *json_escape ( const char *text )
{
	...
	for ( i = 0; i < length; i++ )
	{
		...
		else if ( ( signed char )text[i] < 0 )	/* non-BMP character */
		{
					...
					case 'u':
						{
							...
							{
								fprintf ( stderr, "JSON: unsupported unicode value: %ld\n", unicode );
							}
						}
						break;
					default:
...
int lexer ( const char *buffer, const char **p, unsigned int *state, rcstring **text, size_t *line )
{
	assert ( buffer != NULL );
	assert ( p != NULL );
	assert ( state != NULL );
